...
def actor_ids_from_blade_runner
  ...
  Actor.joins(:movies).where("movies.title = 'Blade Runner'").pluck(:id)
...
def best_years
  ...
  # in which every movie released had a minimum rating of 8 or above.
    ...
    Movie.group(:id).having('movies.score >= 8').pluck(:yr)
...
def harrying_times
  ...
  Movie.joins(:actors).where("actors.name = 'Harrison Ford'").group(:yr).having('COUNT(*) >= 2').pluck(:yr)
